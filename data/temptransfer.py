...
def pull():
    # set up
    allProducts = []
    relprodf = pd.read_csv("sold_id_to_sku.csv")
    columns = ['id', 'sku', 'price', 'name', 'active']

    for index, row in relprodf.iterrows():
        pid = row['id']
        ...
        while ('variant' not in data):
            ...
            time.sleep(10)
            data = pullProduct(pid)
        print("Integrating product with id: " + str(pid) + ", with index number: " + str(index))

        sku = data['variant']['sku']
        price = data['variant']['price']
        name = data['variant']['fully_qualified_name']
        active = data['variant']['active']

        values = [pid, sku, price, name, active]
        allProducts.append(values)

    products = pd.DataFrame(allProducts, columns=columns)
    return products
...
def transfer():
    # set up
    relprodf = pd.read_csv("sold_id_to_sku.csv")
    prodinfodf1 = pd.read_csv("PRODUCTS-forsale.csv")
    prodinfodf2 = pd.read_csv("PRODUCTS-not_forsale.csv")
...
if __name__ =="__main__":
    df = pull()
    df.to_csv(r'products_for_db.csv', index=False, header=True)
